...
## aider か、チャット直接か（使い分け）

**実務ではLM Studioのチャットから直接聞くほうが、回答が分かりやすいことがある。**

| | aider | LM Studioチャット直接 |
|---|---|---|
| 送るプロンプト | リポマップ等を裏で大量に送る | **自分が渡す情報を制御できる** |
| 中規模ローカルモデルとの相性 | 負荷が大きく、暴走もしやすい | 安定しやすい |
| 向く用途 | ファイル編集を任せる | 相談・ログ解析・調べもの |

**「回答が悪い」＝モデルが悪い、とは限らない。経路の問題を先に疑う。**
渡す情報を絞れば、同じモデルでも回答は変わる。
